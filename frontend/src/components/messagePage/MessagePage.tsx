...
const MessagePage = (props: MessagePageProps) => {
    ...
    const onOpenMenu = (e: any) => setAnchor(e.currentTarget);
    ...
    const onChangeSearch = () => setIsSearch(!isSearch);
    const onClearQuery = () => {
        setMessageQuery("");
        setIsSearch(false);
    };
    ...
    const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter') props.onSendMessage();
    };

    const onOpenEmojiPicker = () => setIsEmojiPickerOpen(true);
    const onCloseEmojiPicker = () => setIsEmojiPickerOpen(false);
    ...
    // Filtered or full messages
    const messagesToRender = messageQuery.length > 0
        ? sortedMessages.filter(msg => msg.content.toLowerCase().includes(messageQuery))
        : sortedMessages;
    ...
    return (
        <div className={styles.outerMessagePageContainer}>
            ...
            <div className={styles.messagePageHeaderContainer}>
                <div className={styles.messagePageInnerHeaderContainer}>
                    <div className={styles.messagePageHeaderNameContainer}>
                        <Avatar sx={{ width: '2.5rem', height: '2.5rem', fontSize: '1rem', mr: '0.75rem' }}>
                            {getInitialsFromName(getChatName(props.chat, props.reqUser))}
                        </Avatar>
                        <p>{getChatName(props.chat, props.reqUser)}</p>
                    </div>
                    <div className={styles.messagePageHeaderNameContainer}>
                        {!isSearch && <IconButton onClick={onChangeSearch}><SearchIcon /></IconButton>}
                        {isSearch &&
                            <TextField
                                id='searchMessages'
                                type='text'
                            ...
                            />}
                        <IconButton onClick={onOpenMenu}><MoreVertIcon /></IconButton>
                        <Menu
                            id="basic-menu"
                            anchorEl={anchor}
                            open={open}
                        ...
                        >
                            ...
                            <MenuItem onClick={onDeleteChat}>{props.chat.isGroup ? 'Delete Group Chat' : 'Delete Chat'}</MenuItem>
                        </Menu>
                    </div>
                </div>
            </div>
            ...
            <div className={styles.messageContentContainer} onClick={onCloseEmojiPicker}>
                {messagesToRender.map(msg => getMessageCard(msg))}
            ...
            <div className={styles.footerContainer}>
                {isEmojiPickerOpen ?
                    ...
                    <div className={styles.emojiButton}>
                        <IconButton onClick={onOpenEmojiPicker}><MoodIcon /></IconButton>
                    </div>
                }
                <div className={styles.innerFooterContainer}>
                    <TextField
                        ...
                        InputProps={{
                            endAdornment: (
                                <InputAdornment position='end'>
                                    <IconButton onClick={props.onSendMessage}><SendIcon /></IconButton>
                                </InputAdornment>
                            )
                        }}
                    />
